  ...
  return (
    <div className={`p-6 ${isDarkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-gray-200'} rounded-xl shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300 cursor-pointer`}>
      <div className="flex justify-center mb-4">
        <div className={`w-12 h-12 ${isDarkMode ? 'bg-[#F97316]/30' : 'bg-orange-100'} rounded-full flex items-center justify-center`}>
          {icon}
        </div>
      </div>
      ...
      <p className="text-sm text-[#F97316] font-medium text-center">{duration}</p>
    </div>
  );
}
